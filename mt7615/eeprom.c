					 ...
					 u8 chain_idx)
{
	...
	/* MT_EE_EXT_PA_{2,5}G_TARGET_POWER is not a valid tx target power:
	 * on external PA devices it either includes the PA gain or is left
	 * unprogrammed, so it must not be used as the per-chain reference.
	 */
	if (chan->band == NL80211_BAND_2GHZ) {
